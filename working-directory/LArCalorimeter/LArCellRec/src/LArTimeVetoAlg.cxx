...
StatusCode LArTimeVetoAlg::execute()
{
  ...
  // std::cout << " ABINFO LArTimeVetoAlg " << std::endl;
  ...
  if (true) {
    ...
    if (eventInfo) {
      std::cout << "ABDEBUG: LArTimeVetoAlg r,e=" << eventInfo->runNumber() << "," << eventInfo->eventNumber()
                << " vetoWord=" << std::hex << vetoWord << std::dec
                << " nb=" << eventInfo->isEventFlagBitSet(EventInfo::LAr,LArEventBitInfo::NOISEBURSTVETO)
                << " dc=" << eventInfo->isEventFlagBitSet(EventInfo::LAr,LArEventBitInfo::DATACORRUPTEDVETO)
                << std::endl;
    }
  }
