...
def showEventVetoFolder(dbname,folderName,tag,run1,run2,numberJobSegments,thisJobSegment,levelOfDetail=3,onlyFlag=-1):
    retval=dict() 
    ...
    while itr.goToNext():
        obj=itr.currentRef()
        nobj = nobj + 1
        if numberJobSegments is not None and thisJobSegment is not None:
            if not (nobj % numberJobSegments == thisJobSegment):
                continue
        payload=obj.payload()
...
if __name__=="__main__":
    ...
    for (o,a) in opts:
        if o=="-h":
            ...
            print "-f <folder>"
            print "-n number of segments to split job"
            print "-i segment number"
            sys.exit(0)
        ...
        if o=="-f": folderName=a
        if o=="-n": numberJobSegments=int(a)
        if o=="-i": thisJobSegment=int(a)
    ...
    showEventVetoFolder(db,folderName,tag,run1,run2,numberJobSegments,thisJobSegment,levelOfDetail)
