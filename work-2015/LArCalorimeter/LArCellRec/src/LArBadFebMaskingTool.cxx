...
StatusCode LArBadFebMaskingTool::process(CaloCellContainer * theCont )
{
  ...
  if (nLar==0) {
     ...
     // FIXME: this is a hack for processing TLA jet DataScouting events
     // TODO: add a configuration option to skip this check
     // flagBadEvent = true;
  }
...
// for debug
      ...
      if (it1 != febMap.end()) {
        ...
        ATH_MSG_DEBUG (" ierror,toMask " << ierror << " " << toMask1 << " ");
         std::cout << "ABDEBUG: LArBadFebMaskingTool "
                   << " ierror,toMask " << ierror << " " << toMask1 << " "
                   << std::endl;
      }
  ...
  if (eventInfo) {
    std::cout << "ABDEBUG: LArBadFebMaskingTool r,e=" << eventInfo->runNumber() << "," << eventInfo->eventNumber()
              << " dc=" << flagBadEvent << " tladc=" << (nLar>0 && flagBadEvent)
              << std::endl;
  }

  // FIXME: as above, skip check if nLAr==0
  if (eventInfo && nLar>0 && flagBadEvent) {
